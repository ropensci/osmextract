...
If you want to download your data into a persistent directory, set `OSMEXT_DOWNLOAD_DIRECTORY=/path/for/osm/data` in your `.Renviron` file, e.g. with `usethis::edit_r_environ()` or manually. For example: 
...
```{r}
osmext_get("Isle of Wight", verbose = TRUE)
osmext_get("Isle of Man", verbose = TRUE)
osmext_get(c(9.1916, 45.4650), verbose = TRUE)
```
