...
- Simplified
  ...
  and removed the argument `quiet`. Now the function is always quiet.
  The corresponding message is included in the docs.
- Bump minimum R version to 4.2.0 to simplify the package’s workflow and
  adopt [`...names()`](https://rdrr.io/r/base/dots.html). In fact,
  …names() was introduced in R 4.1.0, but a relevant bug was fixed only
  included from R 4.1.3.
...
- Added a few R packages to `Suggests`, namely `sfnetworks`, `dodgr`,
  and `dplyr`. They are used to implement the new `oe_get_*network()`
  functionalities.
  ([\#321](https://github.com/ropensci/osmextract/issues/261))
...
- Updated the behaviour of
  ...
  …) when a `sf/sfc/bbox` is provided as input for `place`. If
  `boundary` is not `NA`, it is automatically set to match `place` so
  only the relevant portion of the extract is processed
...
- Fixed a bug in
  ...
  that occurred every time the function found an exact match between the
  input `place` and a non-default OSM data provider (i.e. everything but
  Geofabrik). In those cases, the downloaded file was named as
...
- Fixed a bug in
  ...
  that occurred when the value for a given key was either empty or equal
...
- Fixed a bug in
  ...
  that occurred when the attributes specified in the `extra_tags`
  argument included the character `:`. In fact, the presence of
...
- Added a new function named
  [`read_poly()`](https://docs.ropensci.org/osmextract/reference/read_poly.md)
  to read `.poly` files
...
- The function
  [`oe_get_keys()`](https://docs.ropensci.org/osmextract/reference/oe_get_keys.md)
  gains a new argument named `download_directory` that can be used to
  specify the path of the directory that stores the `.osm.pbf` files.
...
- Added a message to
  [`oe_download()`](https://docs.ropensci.org/osmextract/reference/oe_download.md)
  and removed a warning from
  [`oe_read()`](https://docs.ropensci.org/osmextract/reference/oe_read.md).
  The message is printed every time a user downloads a new OSM extract
  from a certain provider, whereas the warning used to be raised when a
  given `query` selected a layer different from the `layer` argument
...
- Added a more informative error message in case
  [`oe_download()`](https://docs.ropensci.org/osmextract/reference/oe_download.md)
  fails, explaining that partially downloaded `.pbf` files should be
  removed to avoid problems while running other functions
...
- The file path returned by
  [`oe_download()`](https://docs.ropensci.org/osmextract/reference/oe_download.md)
  is specified using `/` instead of `\\` separator on Windows.
- [`oe_download()`](https://docs.ropensci.org/osmextract/reference/oe_download.md)
  takes into account the `timeout` option again. Unfortunately, we
  forgot to adjust the code when switching from `download.file` to
  `httr`.
- The
  [`oe_vectortranslate()`](https://docs.ropensci.org/osmextract/reference/oe_vectortranslate.md)
...
- Improved the approach adopted to download files from the web. In
  ...
  [`oe_download()`](https://docs.ropensci.org/osmextract/reference/oe_download.md)
  and
  [`oe_search()`](https://docs.ropensci.org/osmextract/reference/oe_search.md)
  now take advantage of `httr` functionalities. They return informative
...
- Fixed a bug in
  [`oe_vectortranslate()`](https://docs.ropensci.org/osmextract/reference/oe_vectortranslate.md)
  that occurred when reading `multilinestrings` or `other_relations`
  layers with one or more extra tags
  ([\#229](https://github.com/ropensci/osmextract/issues/229)).
- Fixed a bug in
  [`oe_get()`](https://docs.ropensci.org/osmextract/reference/oe_get.md)/[`oe_read()`](https://docs.ropensci.org/osmextract/reference/oe_read.md)
...
- Added a new (but still experimental) function named
  [`oe_get_network()`](https://docs.ropensci.org/osmextract/reference/oe_get_network.md)
  to import a road network used by a specific mode of transport. For the
  moment, we support the following modes of transport: cycling
  (default), walking, and driving. Check
  [`?oe_get_network`](https://docs.ropensci.org/osmextract/reference/oe_get_network.md)
...
- The
  [`oe_get_keys()`](https://docs.ropensci.org/osmextract/reference/oe_get_keys.md)
  function can be used to extract the values associated with all or some
  keys. We also defined an ad-hoc printing method and fixed several
  bugs. The examples were improved. Moreover, the function tries to
  match an input `zone` with one of the OSM extracts previously
  downloaded ([\#201](https://github.com/ropensci/osmextract/issues/201)
  and [\#196](https://github.com/ropensci/osmextract/issues/196)).
- If the parameter `place` represents an `sf`/`sfc`/`bbox` object with
  missing CRS, then
  [`oe_match()`](https://docs.ropensci.org/osmextract/reference/oe_match.md)
  raises a warning message and sets `CRS = 4326`. This has relevant
  consequences on other functions (like
  [`oe_get()`](https://docs.ropensci.org/osmextract/reference/oe_get.md))
  that wrap
  [`oe_match()`](https://docs.ropensci.org/osmextract/reference/oe_match.md).
- Starting from `sf` \> 0.9.8, the function
  [`oe_vectortranslate()`](https://docs.ropensci.org/osmextract/reference/oe_vectortranslate.md)
  stops with an error when there is a problem in the argument
...
- We create two new arguments in
  ...
  and
  [`oe_read()`](https://docs.ropensci.org/osmextract/reference/oe_read.md))
  named `boundary` and `boundary_type`. They can be used to create an
  ad-hoc spatial filter during the vectortranslate operations (and
  create even less verbose specifications in `vectortranslate_options`
  argument). See docs and introductory vignette for more details.
- The argument `provider` was removed from
  [`oe_match_pattern()`](https://docs.ropensci.org/osmextract/reference/oe_match_pattern.md)
  since the function automatically checks all available providers
...
- The parameter `force_vectortranslate` is checked before reading the
  ...
  `TRUE`, then
  [`oe_vectortranslate()`](https://docs.ropensci.org/osmextract/reference/oe_vectortranslate.md)
  doesn’t check the existing layers. This is important for user that run
  [`oe_vectortranslate()`](https://docs.ropensci.org/osmextract/reference/oe_vectortranslate.md)
...
- The arguments passed to
  [`oe_read()`](https://docs.ropensci.org/osmextract/reference/oe_read.md)
  via `...` are compared with the formals of `st_read.character`,
...
- The
  [`oe_get_keys()`](https://docs.ropensci.org/osmextract/reference/oe_get_keys.md)
  function can be applied to `.osm.pbf` objects
...
- Modify the behaviour of
  [`oe_match()`](https://docs.ropensci.org/osmextract/reference/oe_match.md).
  The function checks all implemented providers in case the input
  `place` is not matched with any geographical zone for the chosen
  provider ([\#155](https://github.com/ropensci/osmextract/issues/155)).
