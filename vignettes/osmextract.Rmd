...
1. `oe_get()`: Find, download, translate, and import data, all in one step.
...
As you can see, the name of the most important functions in this package start with `oe_*` prefix, which means that you can easily use auto-completion features (with Rstudio or similar IDE). 
...
Perhaps, the best known bulk OSM data provider is Geofabrik, represented as a `data.frame` in the packaged object `geofabrik_zones`.
...
As shown in the visualisation above of [BBBike.org](https://download.bbbike.org/osm/) zones in Europe, the provider offers rectangular extracts of the major cities.
...
Check the ["Comparing the supported OSM providers"](https://itsleeds.github.io/osmextract/articles/providers_comparisons.html) vignette for some simple guidelines on how to choose the best provider. 

## `oe_match()`: Match OSM extracts

The function `oe_match()` takes in input a string through the parameter `place`, and it returns a named list of length two with the URL and the size (in bytes) of a `.osm.pbf` file representing a geographical zone stored by one of the supported providers. 
The `.pbf` format is a highly optimised binary format used by OSM providers to store and share OSM extracts. For example: 
...
By default, it uses the `name` field and Geofabrik provider, but you can select a different column via the argument `match_by`. 
We refer to providers' help pages for a detailed description of all available fields. 
If you are using Geofabrik, a useful and interesting alternative field is represented by the (unique and unambiguous) `iso3166-1 alpha2` [codes](https://it.wikipedia.org/wiki/ISO_3166-1_alpha-2): 
...
For this reason, we coded a function named `oe_match_pattern()` to explore the matching operations according to a pre-defined pattern. 
For example:
...
You can combine the two approaches to (easily) select a geographical area: 
...
The parameter `max_string_dist` (which defaults to `1`) represents the maximum tolerable distance between the input place and the closest match in `match_by` column before the function prints a warning message or stops with an error. 
...
Finally, if there is no exact match with any of the supported providers and `match_by` argument is equal to `"name"`, then `oe_match()` will use the [Nominatim API](https://nominatim.org/release-docs/develop/api/Overview/) to geolocate the input place and perform a spatial matching operation (introduced below): 
...
The input `place` can also be specified using an `sf` or `sfc` object with arbitrary CRS, as documented in the following example. 
...
If the input `place` intersects multiple geographically nested zones, then the argument `level`, which must be specified as an integer between 1 and 4, is used to choose between them. 
It defaults to `NULL`, meaning that `oe_match()` automatically returns the area with the highest `level`. 
We could roughly say that smaller geographical areas are associated with higher `level`(s) and, for example, for `Geofabrik` data `level = 1` corresponds to continent-size extracts, `2` is for countries, `3` represents the regions and `4` the subregions. 
...
If you specify more than one geometry into the `sf` or `sfc` object, then `oe_match()` will select the area that contains all of them. 
...
We strongly advise you setting a persistent directory since downloading and converting (see the next sub-section) `.pbf` files are expensive operations, that are skipped by `oe_*()` functions if they detect that the requested extract was already downloaded and/or converted.
...
In the second step, the function checks if the new path already exists (using `file.exists()`) and, in that case, it returns it, without downloading anything^[The parameter `force_download` is used to override this behaviour.]. 
...
We can always re-create the `.gpkg` file adding new `tags`: 

```{r}
its_gpkg = oe_vectortranslate(its_pbf, extra_tags = c("bicycle", "foot"))
```

Check the `oe_get()` section for more complex, useful, and realistic use-cases. 
...
`ogr2ogr` can perform various operations during the translation process, such as spatial filters or queries, and these operations are determined by the `vectortranslate_options` argument. 
...
The arguments that are passed to `vectortranslate_options` can also be used to perform queries during the vectortranslate process, as shown in the next Sections. 
...
oe_get("Leeds", quiet = FALSE)
oe_get("Goa", quiet = FALSE)
...
The arguments `osmconf_ini`, `vectortranslate_options`, `query` and `wkt_filter` (the last two arguments are defined in `sf::st_read`) can be used to further optimize the process of getting OSM extracts into R.  
...
First, we load a local copy of the default `osmconf.ini` file, taken from the following [link](https://github.com/OSGeo/gdal/blob/master/gdal/data/osmconf.ini). 
...
while the classical approach (also not run in the vignette) is slower and it provides identical results: 
...
The argument `vectortranslate_options` can also be tuned to perform spatial filter operations during the vectortranslate process. 
The option `-spat`, illustrated in the following example, can be used to filter only those features that intersect a given rectangular bounding box, specified as `c(xmin, ymin, xmax, ymax)`. 
For example:
...
Finally, the options `-clipsrc` and `-clipdst` can be used to perform more complex operations such as spatial filtering with generic polygons or clipping. 
The former option crops the features considering a polygon specified in the original CRS, while the latter manipulates the data after the projection. 
In both cases, the polygon must be specified using the Well Known Text format. 
...
We select and clip only the road segments that intersect a 5 kilometers circular buffer centred in La Valletta, the capital. 
...
The option `-clipdst` says that we want to clip the OSM extract after the reprojection to `EPSG:32633`. 
The function `st_as_text()` converts an `sfg` polygon into Well Known Text format, which is mandatory for the spatial filter. 
Hence, `st_as_text(la_valletta)` specifies the polygon used for clipping. 
The last step may return invalid `LINESTRING` geometries. 
...
<img src="https://user-images.githubusercontent.com/22221146/104240598-9d6fb400-545c-11eb-93b5-3563908ff4af.png" width="90%"/>

The process takes approximately 5 seconds, while the equivalent R code, reported below, is slower: 
...
Moreover, the R code crops the road segments using a rectangular boundary insted of the proper circular polygon: 
...
<img src="https://user-images.githubusercontent.com/22221146/104241581-32bf7800-545e-11eb-896b-3f535dd1af5e.png" width="90%"/>
...
This is the output and, we can see that it doesn't crop the features: 
...
<img src="https://user-images.githubusercontent.com/22221146/104243054-4966ce80-5460-11eb-951b-ca1ce9d09f33.png" width="90%"/>
...
Hence, it may become unfeasible for larger `.pbf` files, probably starting from 500MB. 
We will test more cases and add more benchmarks in the near future. 
On the other side, the syntax is cleaner, the approach is more intuitive and, most importantly, it does not require a new time-consuming `ogr2ogr` conversion every time a user defines a new query. 
For these reasons, this is the suggested approach for querying a medium-size OSM extract.
