...
OSM data is usually described using several [*tags*](https://wiki.openstreetmap.org/wiki/Tags), i.e. pairs of two items: a *key* and a *value*. The code at lines `r osmextract:::get_id_layer("points", readLines(get_default_osmconf_ini()))`, `r osmextract:::get_id_layer("lines", readLines(get_default_osmconf_ini()))`, `r osmextract:::get_id_layer("multipolygons", readLines(get_default_osmconf_ini()))`, `r osmextract:::get_id_layer("multilinestrings", readLines(get_default_osmconf_ini()))`, and `r osmextract:::get_id_layer("other_relations", readLines(get_default_osmconf_ini()))` of the default `CONFIG` file determines, for each layer, which tags are explicitly reported as fields, while all the other tags are stored in the `other_tags` column (see [here](https://gdal.org/en/stable/drivers/vector/osm.html#other-tags-field) for more details). 
