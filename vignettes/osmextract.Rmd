...
This is extremely important since, if we tune the `vectortranslate_options` parameter, then we can analyze very big `.pbf` files without reading them in. 

The following code shows how to modify the CRS of the `.gpkg` object while vectortranslating: 
...
The following chunk (not evaluated in the vignette) shows how to create a spatial query while vectortranslating: 
