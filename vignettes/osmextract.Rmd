...
plot(st_geometry(bbbike_zones), xlim = c(-2, 10), ylim = c(35, 60), add = TRUE, col = "darkred")
```
...
Therefore, you can select alternative fields in the provider's data (instead of the default field, i.e. `name`), that can be specified using the `match_by` parameter. 
...
For this reason, we coded a function that let you explore the matching operations according to a chosen pattern, a provider (default is Geofabrik) and a field (default is `name`). 
For example:
...
Then, you can use the returned values to select the geographical area: 
...
You can always increase this value, but it can be dangerous: 
...
The argument `provider` can be omitted if the input `file_url` is associated with one of the supported providers (such as Geofabrik).
...
First, we download the `.pbf` file associated with the ITS example: 
...
The code at line 7 of that CONFIG file is used to determine which *ways* are assumed to be *polygons* (according to the simple-feature definition of polygon) if they are closed.

The parameter `osmconf_ini` specifies the path to an ad-hoc `CONFIG` file, in case you need full control over GDAL operations. 
See the next section for an example. 
If `osmconf_ini` is equal to `NULL` (the default), then `oe_vectortranslate()` function uses the standard `CONFIG` file (that can always be modified using the `extra_tags`, see next sentences).
...
The parameter `extra_tags` (defaulting to `NULL`) governs which tags are explicitly reported in the `.gpkg` file and are omitted from the `other_tags` field. 
The default tags are always included (unless you modify the `CONFIG` file). 

Lastly, the `oe_get_keys()` function can be used to check all the `keys` that are stored in the `other_tags` field for a given `.gpkg` file. 
For example, 
...
See the next sections for more complex use-cases. 
