...
`oe_match()` will return a named list of length two with the URL and the size of a `.pbf` file representing a zone that geographically intersects the `sfc_POINT` (or an error, if the input point does not cross any area). 
If the input `place` intersects multiple geographically nested zones, the function returns the area with the highest `level`. 
The meaning of the `level` fields depends on the chosen provider, so we refer to the help pages for more details. 
We could roughly say that the higher is the `level`, the lower is the geographical area’s administrative unit.
...
```{r, echo = FALSE}
its_leeds = oe_get("ITS Leeds", force_vectortranslate = TRUE)
