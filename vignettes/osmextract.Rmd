...
all_matches_with_milan = geofabrik_zones[st_transform(milan_duomo, st_crs(geofabrik_zones)), ]
...
We strongly advise you setting a persistent directory since downloading and converting (see the next sub-section) `.pbf` file are expensive operations, that can be avoided by `oe_*()` function if they detect that the input `.pbf` file was already downloaded. 
