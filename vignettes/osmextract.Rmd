...
```{r}
str(geofabrik_zones, max.level = 1, give.attr = FALSE, vec.len = 2)
```
...
By default, `oe_match()` uses the `name` field and the `Geofabrik` provider, but you can select a different field via the argument `match_by`. 
...
As in the previous case, `oe_match()` will return a named list of length two with the URL and the size of a `.pbf` file representing a zone that geographically contains the `sf` or `sfc` object  (or an error in case the input is not contained into any geographical area).
...
We could roughly say that smaller geographical areas are associated with higher `level`(s).
...
Starting from version `0.5`, we can use the function `oe_match_pattern()` to explore all available providers starting from a given spatial pattern

```{r}
westminster_abbey <- c(-0.127, 51.5) 
oe_match_pattern(westminster_abbey)
```
...
In the second step, the function checks if the newly defined file already exists (using `file.exists()`) and, in that case, it returns the path, without downloading anything^[The parameter `force_download` can be used to override this behaviour in case you need to update an old OSM extract. See also `oe_update()`.]. 
Otherwise, it downloads a new file (using ` httr::GET()` with `httr::write_disk()`) and then it returns the path.

## `oe_vectortranslate()`: Convert to gpkg format

The `oe_vectortranslate()` function translates a `.pbf` file into `.gpkg` format^[The GeoPackage (`.gpkg`) is an *open, standards-based, platform-independent, portable, self-descripting, and compact format for transferring geospatial information*. See [here](http://www.geopackage.org/).]. 
...
Starting from `sf` version [0.9.6](https://r-spatial.github.io/sf/news/index.html#version-0-9-6-2020-09-13), if `quiet` argument is equal to `FALSE`, then `oe_vectortranslate()` will display a progress bar during the vectortranslate process.
...
Another relevant parameter that could be customised during the creating of an ad-hoc `osmconf.ini` file is `closed_ways_area_polygons` (see lines 5-7 of the default CONFIG file and the discussion at the following link: <https://github.com/ropensci/osmextract/discussions/228>). We can now write a local copy of the `custom_osmconf_ini` file: 
