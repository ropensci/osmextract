...
In particular, we could roughly say that smaller geographical areas are associated with higher `level`(s) and `level = 1` corresponds to continent-size extracts, `2` is for countries, `3` represents the regions and `4` the subregions. 
For example: 
...
Starting from version `0.3.0`, if you set `values = TRUE`, then `oe_get_keys` prints the values associated to each key: 

```{r}
oe_get_keys(its_gpkg, layer = "lines", values = TRUE)
```

Check `?oe_get_keys` and the corresponding examples for more details. 
...
The utility `ogr2ogr` can perform various operations during the translation process, such as spatial filters or queries, and these operations are determined by the `vectortranslate_options` argument. 
...
The arguments passed to `vectortranslate_options` can also be used to perform various steps during the vectortranslate process, as shown in the next sections. 

### `boundary` and `boundary_type` arguments 

According to our experience, the spatial filters are the most common operations added to the (default) vectortranslate process (usually to select a smaller area lying in a larger OSM extract). 
...
These new arguments are exemplified in the next sections and can help the users creating less verbose `vectortranslate_options`. 
...
If `osmconf_ini`, `vectortranslate_options` or `boundary` parameters are not `NULL`, the vectortranslate operations are never skipped.
