...
test_that("get_default_osmconf_ini returns a valid osmconf.ini file", {
  result <- get_default_osmconf_ini()
  expect_type(result, "character")
  expect_length(result, 1L)
  expect_true(file.exists(result))
  lines <- readLines(result, warn = FALSE)
  # osmconf.ini has exactly 5 layers (points, lines, multipolygons,
  # multilinestrings, other_relations) each with one "attributes=" line
  expect_equal(sum(grepl("^attributes=", lines)), 5L)
})

test_that("get_default_osmconf_ini falls back to sf-bundled file when gdal-config fails", {
  withr::local_bindings(
    system2 = function(command, args, stdout, ...) stop("mocked failure"),
    .env = asNamespace("osmextract")
  )
  result <- suppressWarnings(get_default_osmconf_ini())
  expect_type(result, "character")
  expect_length(result, 1L)
})

test_that("get_default_osmconf_ini warns and uses package bundled file when sf file unavailable", {
  withr::local_bindings(
    system2 = function(command, args, stdout, ...) stop("mocked failure"),
    system.file = function(..., package = "base", lib.loc = NULL, mustWork = FALSE) {
      if (identical(package, "sf")) return("")
